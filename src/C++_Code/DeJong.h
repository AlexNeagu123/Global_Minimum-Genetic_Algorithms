...
class DeJong : public GenericFunction {
	std::pair<double, double> domain;
	const double epsilon;
	int dimension;
public:
	BitmaskDecoder decoder;
	explicit DeJong(const std::pair<double, double>& _domain, int _dimension, double _epsilon) : domain(_domain), epsilon(_epsilon), dimension(_dimension), decoder(domain, epsilon, dimension) {
