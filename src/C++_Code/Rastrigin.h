...
class Rastrigin : public GenericFunction {
	std::pair<double, double> domain;
	const double epsilon;
	int dimension;
public:
	BitmaskDecoder decoder;
	explicit Rastrigin(const std::pair<double, double>& _domain, int _dimension, double _epsilon) : domain(_domain), epsilon(_epsilon), dimension(_dimension), decoder(domain, epsilon, dimension) {
	...
	std::vector<double> get_pc() override;
	std::vector<double> get_pm() override;
};